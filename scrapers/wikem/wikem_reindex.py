#!/usr/bin/env python3
"""
WikEM RAG Re-Indexer
Reusable script to rebuild the WikEM Vertex AI RAG corpus from scratch.
...
Designed to be re-run every ~6 months when WikEM is re-scraped.

Usage:
    # Full rebuild (delete old corpus, create new, upload & index)
    python wikem_reindex.py

    # Upload to GCS only (no corpus changes)
    python wikem_reindex.py --upload-only

    # Skip GCS upload, just rebuild corpus from existing GCS files
    python wikem_reindex.py --skip-upload

    # Dry run — show what would happen
    python wikem_reindex.py --dry-run
"""
...
GCS_BUCKET_NAME = f"{PROJECT_ID}-wikem"
...
CONFIG_PATH = SCRIPT_DIR / "wikem_rag_config.json"
...
log = logging.getLogger("wikem-reindex")
...
def delete_corpus(corpus_name: str, dry_run: bool = False) -> bool:
    ...
    # Force delete even if it has files
...
def import_gcs_to_corpus(corpus_name: str, dry_run: bool = False) -> bool:
    ...
    if op_name:
        ...
        import_result = _poll_operation(op_name, description="import files", max_wait=1800)
        if import_result:
            # Try to extract import stats
...
def _poll_operation(op_name: str, description: str = "operation", max_wait: int = 300) -> dict | None:
    ...
    # Handle fully-qualified vs relative operation names
...
def reindex(
    ...
    if not dry_run and corpus_id and corpus_id != old_config.get("corpus_id"):
        print("⚠️  CORPUS ID CHANGED — update Cloud Run env var:")
        print(f"  gcloud run services update em-protocol-api \\")
        print(f"    --region us-central1 \\")
        print(f"    --update-env-vars WIKEM_CORPUS_ID={corpus_id}")
        print()
        print("  Or set it in the Cloud Console:")
        print(f"  WIKEM_CORPUS_ID = {corpus_id}")
        print()
...
def main():
    parser = argparse.ArgumentParser(
        description="WikEM RAG Re-Indexer — rebuild the corpus from scratch",
