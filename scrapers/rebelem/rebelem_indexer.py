#!/usr/bin/env python3
"""
REBEL EM RAG Indexer
Creates a separate Vertex AI RAG corpus for REBEL EM content
and indexes scraped markdown files into it.

Usage:
    # Create the REBEL EM corpus (one-time)
    python rebelem_indexer.py --create-corpus

    # Index a single file
    python rebelem_indexer.py --index-file output/processed/acute-pancreatitis.md

    # Index all processed markdown files
    python rebelem_indexer.py --index-all

    # Check corpus status
    python rebelem_indexer.py --status

    # List indexed files
    python rebelem_indexer.py --list-files

    # Validate all files are indexed
    python rebelem_indexer.py --validate
"""
...
GCS_BUCKET_NAME = f"{PROJECT_ID}-rebelem"

CORPUS_DISPLAY_NAME = "rebelem"
CORPUS_DESCRIPTION = (
    "REBEL EM (Rational Evidence Based Evaluation of Literature in Emergency Medicine) "
    "— evidence-based reviews of recent EM literature with clinical bottom lines "
    "and critical appraisals from rebelem.com. CC BY-NC-ND 4.0."
)
...
CONFIG_PATH = Path(__file__).parent / "rebelem_rag_config.json"
...
log = logging.getLogger("rebelem-indexer")
...
def create_corpus() -> dict:
    """Create a new RAG corpus for REBEL EM content."""
...
def get_corpus_status() -> dict:
    """Get the status of the REBEL EM corpus."""
    config = load_config()
    if not config:
        return {}
...
def list_rag_files() -> list:
    """List all files indexed in the REBEL EM corpus."""
    config = load_config()
    if not config:
        return []
...
def index_file(md_path: Path) -> bool:
    """
    Index a single markdown file into the REBEL EM RAG corpus.
...
def main():
    parser = argparse.ArgumentParser(description="REBEL EM RAG Indexer")
    group = parser.add_mutually_exclusive_group(required=True)
    group.add_argument("--create-corpus", action="store_true", help="Create the REBEL EM RAG corpus")
    ...
    if args.create_corpus:
        ...
        if result:
            ...
            print(f"\n💡 Next step: python rebelem_indexer.py --index-all")
        else:
