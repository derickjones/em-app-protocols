#!/usr/bin/env python3
"""
REBEL EM Bulk Scraper
Parallel scraper with progress tracking, resume support, and error retry.

Usage:
    python rebelem_bulk_scrape.py                     # Scrape all discovered pages
    python rebelem_bulk_scrape.py --limit 50          # Scrape first 50 pages
    python rebelem_bulk_scrape.py --workers 3         # Use 3 parallel workers
    python rebelem_bulk_scrape.py --resume            # Resume from last run
    python rebelem_bulk_scrape.py --retry-errors      # Retry previously failed pages
    python rebelem_bulk_scrape.py --force             # Re-scrape already completed pages
    python rebelem_bulk_scrape.py --no-gcs            # Skip GCS uploads (local only)
"""
...
from rebelem_scraper import extract_page

# Configuration
DISCOVERY_PATH = Path(__file__).parent / "discovery" / "rebelem_discovery.json"
...
# REBEL EM robots.txt: Crawl-delay: 10
# With 3 workers, each worker gets ~3.3s effective delay per request
DEFAULT_WORKERS = 3
REQUEST_DELAY = 3.0  # Per-worker delay (on top of the 10s in-scraper delay)
...
def main():
    parser = argparse.ArgumentParser(description="REBEL EM Bulk Scraper")
    ...
    parser.add_argument("--delay", type=float, default=REQUEST_DELAY,
                        help=f"Per-worker delay between requests (default: {REQUEST_DELAY})")

    args = parser.parse_args()

    # Load discovery data
    if not DISCOVERY_PATH.exists():
        logger.error("No discovery data found. Run rebelem_discovery.py --sitemap first.")
        return
    ...
    logger.info(f"  Crawl-delay: 10s (in-scraper) + {args.delay}s (per-worker)")
    ...
    with ThreadPoolExecutor(max_workers=args.workers) as executor:
        ...
        for future in as_completed(futures):
            ...
            # Rate limiting between worker completions
    ...
    if tracker.stats.get("started_at"):
        ...
        delta = now - start
        elapsed = f" in {delta.total_seconds() / 60:.1f} min"

    print(f"\n{'='*60}")
    print(f"REBEL EM BULK SCRAPE COMPLETE{elapsed}")
    print(f"{'='*60}")
    print(f"  Total:       {total}")
